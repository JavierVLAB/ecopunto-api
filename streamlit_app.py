...
st.markdown("# App en mantenimiento")

# warnings.filterwarnings("ignore", category=matplotlib.MatplotlibDeprecationWarning)

# is_local = False
# firebase_creds_path = "secrets/ecopunto.json"
# enable_processing = True  # Variable para habilitar/deshabilitar el procesamiento
...
# def download_backup_items_json():
#     collection_ref = db.collection("events")
#     query = collection_ref.where("event_name", "==", "Backup")
#     docs = query.stream()
...
#     for doc in docs:
#         item = doc.to_dict()
#         # Convertir el timestamp a un objeto datetime para poder ordenar
#         if "timestamp" in item:
#             item["parsed_timestamp"] = datetime.fromisoformat(item["timestamp"].replace("Z", "+00:00"))
#         backup_items.append(item)

#     # Ordenar los items por el campo 'parsed_timestamp'
#     backup_items.sort(key=lambda x: x["parsed_timestamp"] if "parsed_timestamp" in x else datetime.min)

#     # Eliminar el campo auxiliar 'parsed_timestamp' antes de guardar
#     for item in backup_items:
#         item.pop("parsed_timestamp", None)

#     # Escribir los datos en un archivo JSON
#     json_file_path = "backup_items.json"
#     with open(json_file_path, mode='w', encoding='utf-8') as file:
#         json.dump(backup_items, file, ensure_ascii=False, indent=4)

#     return json_file_path

# def download_all_items():
#     db = firestore.client()
#     collection_ref = db.collection('events')  # Reemplaza 'your_collection_name' con el nombre de tu colección
...
#     if st.button("Download Backup Items"):
#         file_path = download_backup_items_json()
#         st.success(f"Items descargados en: {file_path}")
#         st.download_button(
#             label="Descargar archivo",
#             data=open(file_path, "rb").read(),
#             file_name="backup_items.csv",
#             mime="text/csv"
#         )

#     if st.button("Download All Items"):
#         file_path = download_all_items()
#         st.success(f"Todos los ítems descargados en: {file_path}")
#         st.download_button(
#             label="Descargar todos los ítems",
#             data=open(file_path, "rb").read(),
#             file_name="all_items.json",
